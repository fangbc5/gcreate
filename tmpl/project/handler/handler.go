func (h Handler) {{Code|capfirst}}(c *gin.Context) {
	...
	{% for busiFlow in BusiFlows %}{% if busiFlow.Code|lower == OutputParamsCode|lower %}{{busiFlow.Code|lower}} := {% endif %}h.BusiFlow.{{busiFlow.Code|capfirst}}({% if busiFlow.InputParamsCode != "" %}{{busiFlow.InputParamsCode|lower}}{% endif %})
	{% endfor %}
